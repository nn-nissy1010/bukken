...
- `admin.php` … 管理画面／ネットワーク管理の整理、新規物件サイトの自動設定、カスタマイザー（メインビジュアル）、NS Cloner日本語化、**アップロード上限の引き上げ**（画像・PDF共通で約30MB）、ダッシュボード案内＋公開前チェックウィジェット。固定ページメニューは非表示（誤操作防止）だが、本文編集への導線は「物件基本設定›ページ本文の編集」（`mrc_render_edit_pages_page`・`mrc_editable_body_pages()`）とダッシュボードのクイックリンクで提供。
