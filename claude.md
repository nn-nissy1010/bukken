...
- `meta-boxes.php` … 資料PDF・動画URLのメタボックスと、埋め込み／サムネ／ダウンロードURLのヘルパー。ブロックエディタ無効化（news/document/video/qa はクラシック編集）。**種別（news_category／doc_category）は単一選択ラジオ**（`meta_box_cb` に `mrc_radio_tax_meta_box`、保存は `mrc_save_radio_tax` が1件に置換）。
...
- **工事の計画の編集**：専用フォーム（サイドバー「ページ編集›工事の計画」＝`mrc_render_plan_edit_page`／`mrc_plan_content`）で編集。リード文・各見出し・本文に加え、**目的カード／対象箇所／工程ステップは行の追加・削除ができる可変リスト**。`page-plan.php` は `mrc_get_plan_content()` から取得して**見た目（カード・工程ステップ）を担保**し、テキストだけ差し替える（生HTML編集はしない＝KSES/wpautopの心配なし）。未入力の見出し等は `mrc_default_plan_content()` にフォールバック。目的カードのアイコンは順番に `.purpose-icon--safe/--value/--home` をCSS(mask)で割当。資料ダウンロード欄は常時自動表示。
- **固定ページ本文の差し替え（contact / contact-public）**：`page-contact.php`／`page-contact-public.php` は固定ページ本文が入力されていればそれを表示、未入力なら定型文を表示（`inc/setup.php` の `mrc_has_page_body()`／`mrc_the_page_body()`）。フォームなどの機能部品は常時表示。`page-member.php` も同ヘルパーで冒頭に任意の案内ブロックを出せる（既定は無し＝ダッシュボード型）。標準文の投入は `mrc_standard_page_content($slug)`＋`deploy/seed-standard-page-content.php`（冪等・contact/contact-public のみ）。ブロックエディタは無効（クラシックエディタ）。
